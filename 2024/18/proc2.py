...
rain_bytes = [tuple(map(int, line.split(","))) for line in lines]
prev_walk = None
for idx, rain_coords in enumerate(rain_bytes):
    print("===== trying", idx, rain_coords)
    xmap[rain_coords] = CORRUPTED
    if prev_walk is not None and rain_coords not in prev_walk:
        # the previous walk, which got to the end, didn't pass for the tile about to be corrupted,
        # so for sure we can get to the end if this tile gets corrupted
        print("=====    !")
        continue
    ...
    if ended is None:
        break
    end_len, prev_walk = ended
    print("=====    got", end_len)
else:
    raise ValueError("Never interrupted!")
print(xmap.show())
total = rain_coords
