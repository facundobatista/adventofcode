...
def _get_type(hand):
    ...
    if len(cnt) == 1:
        ...
        # 4 & 1, or 3 & 2
        if max(cnt.values()) + j_quant == 4:
            result = 6  # four of a kind
        else:
            result = 5  # full house
    elif len(cnt) == 3:
        # 3 & 1 & 1, or 2 & 2 & 1
        if max(cnt.values()) + j_quant == 3:
...
CARD_ORDER = list(reversed("AKQT98765432J"))
...
for rank, (hand, bid) in enumerate(allgame, 1):
    print("===== calc", rank, hand, bid, _get_type(hand))
    total += rank * bid
