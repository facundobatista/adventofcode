...
def intermediate(board_name, topress):
    print("============ intermediate", board_name, topress)
    ...
    print("========= partial SUBseqs", allseqs)
    return allseqs


def keyin(board_name, topress):
    print("====== keying", board_name, topress)
    board = BOARDS[board_name]
    cur_x, cur_y = board["A"]  # always starts in the A
    allseqs = []

    for subseq in topress:
        # print("========= nxt sub", subseq)
        seqs = intermediate(board_name, subseq)
        allseqs.extend(seqs)
    # print("========= partial !!!seqs", allseqs)
    print("========= partial !!!seqs", len(allseqs))
    ...
    for combin in itertools.product(*allseqs):
        ...
        # for seq in combin:
        #     fullseq.extend(seq)
        combinseqs.append(tuple(tuple(seq) for seq in combin))
    # print("========= combind seqs", combinseqs)
    print("========= combind seqs", len(combinseqs))
...
for srcseq in lines:
    ...
    for layer in range(2, robot_layers + 2):
        ...
        for seq in produced_seqs:
            ...
            # print("================== PART", layer, len(produced_seqs))
            next_seqs.update(produced_seqs)
        produced_seqs = next_seqs
        print(f"================== PART {layer} total", len(produced_seqs))

    # get the shortest
    sample = min(sum(len(subseq) for subseq in seq) for seq in produced_seqs)
