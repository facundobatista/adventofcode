...
# preprocess lines to make it wider
...
def find_next_empty_horizontally(rob_x, rob_y, dx, dy):
    ...
    current_layer = [(rob_x, rob_y)]

    while True:
        print("======= finding horizontally, layer", current_layer)
        next_layer = set()
        for cur_x, cur_y in current_layer:
            nx = cur_x + dx
            ny = cur_y + dy
            nchar = xmap[(nx, ny)]
            if nchar == WALL:
                ...
                return
            if nchar == BOX_L:
                # pushing a box on one side, annotate both sides and skip it
                to_later_move.append((nx, ny))
                to_later_move.append((nx + 1, ny))
                next_layer.add((nx + 1, ny))
            elif nchar == BOX_R:
                # pushing a box on one side, annotate both sides and skip it
                to_later_move.append((nx, ny))
                to_later_move.append((nx - 1, ny))
                ...
                # empty, nothing to add to next layer
                assert nchar == EMPTY, repr(char)
        print("======= finding, next?", next_layer)
        if not next_layer:
            # done!
            print("============ done!", to_later_move)
...
def find_next_empty_vertically(rob_x, rob_y, dx, dy):
    """Finds next empty element between robot and a wall.
    ...
    while True:
        print("======= finding vertically, layer", current_layer)
        next_layer = set()
        for cur_x, cur_y in current_layer:
            nx = cur_x + dx
            ny = cur_y + dy
            ...
            if nchar == WALL:
                ...
                return
            if nchar == BOX_L:
                # pushing a box on one side, complete with the other side
        ...
        # keep going; anotate next layer for later moving and analyse it
        to_later_move.extend(next_layer)
        current_layer = next_layer
...
for mov in movements:
    ...
    if dx == 0:
        to_move = find_next_empty_vertically(rob_x, rob_y, dx, dy)
    else:
        to_move = find_next_empty_horizontally(rob_x, rob_y, dx, dy)
    print("==========  ??", to_move)
    if to_move is None:
        continue
