...
def keyin(board, topress):
    ...
    final = []
    for partial in allseqs:
        final.extend(partial[0])
    return [tuple(final)]
...
for srcseq in lines:
    ...
    for layer in range(2, robot_layers + 2):
        ...
        minlen = min(len(seq) for seq in next_seqs)
        reduced_seqs = [seq for seq in next_seqs if len(seq) == minlen]
        print(f"================== PART {layer} total", len(next_seqs), len(reduced_seqs), reduced_seqs)
        produced_seqs = reduced_seqs
