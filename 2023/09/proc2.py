...
def extrapolate(sequence):
    ...
    # reduce, keeping first numbers
    firstnums = [sequence[0]]
    while True:
        sequence = [b - a for a, b in sliding_window(sequence, 2)]
        firstnums.append(sequence[0])
        print("========= reduc", sequence)
        if set(sequence) == {0}:
            break

    # build up
    print("===== first nums", firstnums)
    extrapolated = functools.reduce(lambda a, b: b - a, reversed(firstnums))
